<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <link rel="stylesheet" type="text/css" media="screen" href="notconfirmed.css" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link rel="stylesheet" href="../../bootstrap-dist/css/bootstrap.min.css" integrity="sha384-GJzZqFGwb1QTTN6wy59ffF1BuGJpLSa9DkKMp0DgiMDm4iYMj70gZWKYbI706tWS" crossorigin="anonymous">
</head>
<body>

<div class="ConfirmationMessage">
    <span>
    Извините, ваша учётная запись на данный момент не подтверждена администратором.
    <br>
    Ваша заявка будет рассмотрена в течение 2 дней.
    </span>
<p class="logout"><a href="../../Logout/Logout.php">Выйти</a> из системы</p>
</div>

</body>
</html>